...
set_false_path -through [get_nets -hier -filter {name =~*/SiTCP_INT_REG/reg*}]


#set_false_path -from [get_nets u_MTX1/reg_fbh*] -to [get_nets u_MTX1/gen_tof[*].gen_ch[*].u_Matrix_Impl/in_fbh]

# Clock Wizard
create_generated_clock -name clk_sys [get_pins clk_wiz_sys/inst/mmcm_adv_inst/CLKOUT0]
create_generated_clock -name clk_spi [get_pins clk_wiz_sys/inst/mmcm_adv_inst/CLKOUT1]

create_generated_clock -name clk_gmii1 [get_pins network/wrap_gig_eth/core_clocking_i/mmcm_adv_inst/CLKOUT0]
create_generated_clock -name clk_gmii2 [get_pins network/wrap_gig_eth/core_clocking_i/mmcm_adv_inst/CLKOUT1]

set_clock_groups -name async_sys_gmii -asynchronous -group {clk_sys} -group {clk_gmii1 clk_gmii2} -group {clk_spi}

set_false_path -through [get_ports {o_led[1]}]
set_false_path -through [get_ports {o_led[2]}]
set_false_path -through [get_ports {o_led[3]}]
set_false_path -through [get_ports {o_led[4]}]
